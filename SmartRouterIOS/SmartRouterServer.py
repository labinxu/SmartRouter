...
class RouterDummy(SocketServer.BaseRequestHandler):
    ...
    def deviceList(self):
        ...
        time.sleep(1)
        self.request.send(self.generateFanOn(1))
        time.sleep(1)
        self.request.send(self.generateFanOff(2))

    def generateFanOn(slef, index):
        d = Device(0x20, index, b'\x02', 1, 3, 3600000)
        return d.toBytes()
    
    def generateFanOff(slef, index):
        d = Device(0x20, index, b'\x02', 2, 3, 3600000)
        return d.toBytes()
